---
title: API Graph Automated
aliases:
  - API Graph Automated
tags:
  - diagram
  - automation
  - knowledge-1.2
generated: 2026-07-22
sprint: Knowledge 1.2
---
# API Graph Automated

## Overview
Auto-regenerated Mermaid diagram (API Graph Automated) by Documentation Assistant.

## Architecture
```mermaid
flowchart TB
  GW[Gateway]
    GW --> N0["agro_marketplace: /api/agro/v1"]
  GW --> N1["ecosystem: /api/ecosystem/v1"]
  GW --> N2["platform_core: /api/v1"]
  GW --> N3["port_erp: /api/port/v1"]

```

## Components
- Generated from current module/API/agent scan

## Relationships
[[ARCHITECTURE_DASHBOARD]] · [[diagrams/PLATFORM_GRAPH]] · [[build_graph]]

## Responsibilities
Keep graphs synchronized with repository structure.

## Interfaces
`python3 knowledge/tools/build_graph.py`

## REST APIs
API graph reflects discovered prefixes only

## Events
mermaid_regenerated

## Future roadmap
[[ROADMAP]]

## References
[[automation/DOCUMENTATION_ASSISTANT]]

## Related pages
[[INDEX]] · [[DASHBOARD]]
