"""Recruiting Ops repository — generic kind-discriminated registry."""
...
from services.recruiting_ops.models import RecruitingOpsRecord


def record_to_dict(row: RecruitingOpsRecord) -> dict[str, Any]:
    data = dict(row.payload or {})
    data["id"] = str(row.id)
    data["organization_id"] = row.organization_id
    data["tenant_id"] = row.tenant_id
    data["status"] = data.get("status") or row.status
...
class RecruitingOpsRepository:
    def __init__(self, session: AsyncSession) -> None:
        self.session = session

    async def list_kind(self, organization_id: str, kind: str) -> list[RecruitingOpsRecord]:
        result = await self.session.execute(
            select(RecruitingOpsRecord)
            .where(
                RecruitingOpsRecord.organization_id == organization_id,
                RecruitingOpsRecord.kind == kind,
            )
            .order_by(RecruitingOpsRecord.created_at.desc())
        )
        return list(result.scalars().all())

    async def get(self, organization_id: str, item_id: str) -> RecruitingOpsRecord | None:
        try:
            uid = uuid.UUID(str(item_id))
        except ValueError:
            return None
        result = await self.session.execute(
            select(RecruitingOpsRecord).where(
                RecruitingOpsRecord.organization_id == organization_id,
                RecruitingOpsRecord.id == uid,
            )
        )
        return result.scalar_one_or_none()

    async def insert(self, kind: str, data: dict[str, Any]) -> RecruitingOpsRecord:
        payload = {k: v for k, v in data.items() if k not in {"id", "created_at", "updated_at"}}
        row = RecruitingOpsRecord(
    ...
    async def update(self, row: RecruitingOpsRecord, patch: dict[str, Any]) -> RecruitingOpsRecord:
