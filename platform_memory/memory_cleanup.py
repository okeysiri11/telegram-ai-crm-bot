...
class MemoryCleanup:
    def purge_session(self, principal: MemoryPrincipal, *, older_than_hours: float = 24) -> dict[str, Any]:
        ...
        removed = 0
        for r in list(
            continuity_store.list_for(principal.owner_id, level="session", limit=1000, principal=principal)
        ):
            if r.created_at < cutoff and can_delete(principal, r):
                continuity_store.remove(r.id, principal=principal)
                removed += 1
    ...
    def purge_unpinned_working(self, principal: MemoryPrincipal, *, older_than_days: float = 14) -> dict[str, Any]:
        ...
        removed = 0
        for r in list(
            continuity_store.list_for(principal.owner_id, level="working", limit=1000, principal=principal)
        ):
    ...
    def clear_owner(self, principal: MemoryPrincipal) -> int:
        n = 0
        for r in list(continuity_store.list_for(principal.owner_id, limit=5000, principal=principal)):
