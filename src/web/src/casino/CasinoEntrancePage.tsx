import { Link, useOutletContext } from "react-router-dom";
import { fetchCasinoRooms } from "./casinoApi";
import { formatPlayBalance } from "./currency";
import { useEffect, useState } from "react";
import { useCasinoWallet } from "./useCasinoSession";

export function CasinoEntrancePage() {
  const { wallet } = useOutletContext<{ wallet: ReturnType<typeof useCasinoWallet> }>();
  const [live, setLive] = useState(0);
  const [tables, setTables] = useState(1);

  useEffect(() => {
    fetchCasinoRooms("odessa-prime")
      .then((rooms) => {
        setLive(rooms.online_count);
        setTables(rooms.tables.filter((t) => !t.coming_soon).length || 1);
      })
      .catch(() => undefined);
  }, []);

  return (
    <section className="op-entrance" aria-label="Вход Odessa Prime">
      <div className="op-scene">
        <div className="op-layer op-layer-back" />
        <div className="op-layer op-layer-hall" />
        <div className="op-chandelier" aria-hidden />
        <div className="op-columns" aria-hidden>
          <span className="op-col" />
          <span className="op-col" />
          <span className="op-col" />
          <span className="op-col" />
        </div>
        <div className="op-tables-depth" aria-hidden>
          <span className="op-felt-oval" />
          <span className="op-felt-oval" />
          <span className="op-felt-oval" />
        </div>
        <div className="op-marble" />
      </div>
      <div className="op-hero">
        <p className="op-kicker">ОДЕССА · MONACO HALL</p>
        <h1 className="op-title">ODESSA PRIME CASINO</h1>
        <p className="op-sub">Европейская рулетка · только PLAY / DEMO CHIPS</p>
        <div className="op-cta-row">
          <Link className="op-cta" to="/casino/floor">
            ВОЙТИ В КАЗИНО
          </Link>
          <Link className="op-cta secondary" to="/casino/games">
            ВЫБРАТЬ ИГРУ
          </Link>
          <Link className="op-cta secondary" to="/enterprise-city?building=casino">
            ВЕРНУТЬСЯ В ГОРОД
          </Link>
        </div>
        <div className="op-stats">
          <div>
            LIVE PLAYERS
            <b>{live}</b>
          </div>
          <div>
            DEMO BALANCE
            <b>{wallet.wallet ? formatPlayBalance(wallet.wallet.balance_chips) : "—"}</b>
          </div>
          <div>
            ACTIVE TABLES
            <b>{tables}</b>
          </div>
        </div>
      </div>
    </section>
  );
}
