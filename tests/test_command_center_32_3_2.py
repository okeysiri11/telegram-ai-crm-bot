...
def test_platform_version_32_3_2():
    ...
    assert "Enterprise Dashboard" in health["release_status"]
...
def test_config_manifest_32_3_2():
    ...
    assert 'sprint: "32.3.2"' in web
    types = (ROOT / "src" / "web" / "platform-builder" / "types.ts").read_text()
    assert 'PLATFORM_BUILDER_VERSION = "1.44.0"' in types
    assert 'PLATFORM_BUILDER_SPRINT = "32.3.2"' in types
