...
def test_version_and_release_status():
    assert DEFAULT_CONFIG.application_version == "1.1.0-alpha"
    assert DEFAULT_CONFIG.release_status == "Marketplace Alpha"
...
async def test_release_report_production_ready():
    ...
    assert report.application_version == "1.1.0-alpha"
    assert report.release_status == "Marketplace Alpha"
...
def test_release_manifest():
    ...
    assert manifest["application_version"] == "1.1.0-alpha"
    assert manifest["release_status"] == "Marketplace Alpha"
...
async def test_ops_api(client: TestClient):
    ...
    assert data["application_version"] == "1.1.0-alpha"
