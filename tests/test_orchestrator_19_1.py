...
def test_version_orchestrator_ready():
    ...
    assert health["application_version"] == "5.3.1-enterprise"
    assert health["enterprise_foundation"] == "Enterprise Platform v5.3.0-enterprise"
...
def test_decisions_bootstrap():
    ...
    assert boot["version"] == "5.3.1-enterprise"
...
async def test_api_orchestrator(client):
    ...
    assert body["application_version"] == "5.3.1-enterprise"
    ...
    assert (await hub.json())["application_version"] == "5.3.1-enterprise"
...
def test_docs_and_regression_19_1():
    ...
    assert "5.3.1-enterprise" in manifest
    assert "19.1" in manifest
