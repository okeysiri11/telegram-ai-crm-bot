"""Tests — Legal Enterprise Certification & Production Release (Sprint 17.8)."""
...
PREFIX = "/api/legal-enterprise-certification/v1"
EI = "/api/legal-ei/v1"
AA = "/api/legal-aa/v1"
CP = "/api/legal-cp/v1"
DI = "/api/legal-di/v1"
...
def test_full_certification_pack():
    ...
    assert result["version_manifest"]["application_version"] == "5.0.0-enterprise"
...
@pytest.mark.asyncio
async def test_api_legal_enterprise_certification(client):
    ...
    assert body["legal_enterprise_suite_released"] is True

    for prefix in (EI, AA, CP, DI, CM, JI, LI, CE):
...
def test_docs_and_regression_17_8():
    for name in (
        "LEGAL_ENTERPRISE_ARCHITECTURE.md",
        "LEGAL_ENTERPRISE_API.md",
        "LEGAL_ENTERPRISE_DEPLOYMENT.md",
        "LEGAL_ENTERPRISE_SECURITY.md",
        "LEGAL_ENTERPRISE_TEST_REPORT.md",
        "LEGAL_ENTERPRISE_RELEASE_NOTES.md",
    ):
        assert (ROOT / "docs" / name).exists()
    assert (ROOT / "knowledge" / "applications" / "LEGAL_ENTERPRISE_CERTIFICATION.md").exists()
    assert (ROOT / "applications" / "legal_enterprise" / "enterprise_certification" / "facade.py").exists()
    for pkg in (
        ...
        assert (ROOT / "applications" / "legal_enterprise" / pkg / "facade.py").exists()
    for artifact in (
        ...
        assert (ROOT / "applications" / "legal_enterprise" / "release" / artifact).exists()
    ...
    notes = (ROOT / "applications" / "legal_enterprise" / "release" / "RELEASE_NOTES.md").read_text()
    assert "5.0.0-enterprise" in notes
