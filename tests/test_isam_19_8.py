...
def test_identity_auth_rbac_session():
    ...
    sess = suite.sessions.create(identity_id=user["identity_id"], device="linux", ip="128.0.0.1")
...
async def test_api_isam(client):
    ...
    for prefix in (HUB, ORCH, KG, AA, CM, WF, EIP, EDP):
        ...
        assert (await resp.json())["application_version"] == "8.0.0"
