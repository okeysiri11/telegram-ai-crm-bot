...
def test_version_isam_ready():
    ...
    assert health["application_version"] == "5.3.9-enterprise"
    assert health["enterprise_foundation"] == "Enterprise Platform v5.3.8-enterprise"
...
def test_mfa_monitor_audit_bootstrap():
    ...
    assert boot["version"] == "5.3.9-enterprise"
...
async def test_api_isam(client):
    ...
    assert body["application_version"] == "5.3.9-enterprise"
    ...
    for prefix in (HUB, ORCH, KG, AA, CM, WF, EIP, EDP):
        ...
        assert (await resp.json())["application_version"] == "5.3.9-enterprise"
...
def test_docs_and_regression_19_8():
    ...
    assert "5.3.9-enterprise" in manifest
    assert "19.9" in manifest
