...
def test_version_judicial_intelligence_ready():
    ...
    assert health["application_version"] == "4.9.2-enterprise"
    assert health["enterprise_foundation"] == "Enterprise Platform v4.9.1-enterprise"
...
def test_analysis_analytics_and_bootstrap():
    ...
    assert boot["version"] == "4.9.2-enterprise"
...
async def test_api_judicial_intelligence(client):
    ...
    assert body["application_version"] == "4.9.2-enterprise"
    ...
    for prefix in (PREFIX, LI):
        ...
        assert (await resp.json())["application_version"] == "4.9.2-enterprise"
...
def test_docs_and_regression_17_2():
    ...
    assert "4.9.2-enterprise" in manifest
    assert "17.2" in manifest
