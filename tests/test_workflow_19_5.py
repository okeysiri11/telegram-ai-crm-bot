...
def test_version_workflow_ready():
    ...
    assert health["application_version"] == "5.3.7-enterprise"
    assert health["enterprise_foundation"] == "Enterprise Platform v5.3.6-enterprise"
...
def test_templates_optimization_bootstrap():
    ...
    assert boot["version"] == "5.3.7-enterprise"
...
async def test_api_workflow(client):
    ...
    assert body["application_version"] == "5.3.7-enterprise"
    ...
    for prefix in (HUB, ORCH, KG, AA, CM):
        ...
        assert (await resp.json())["application_version"] == "5.3.7-enterprise"
...
def test_docs_and_regression_19_5():
    ...
    assert "5.3.7-enterprise" in manifest
    assert "19.7" in manifest
