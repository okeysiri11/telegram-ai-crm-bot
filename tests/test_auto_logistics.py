...
def test_version_modules_docs_bridges():
    ...
    assert docs.exists() and "1.5.0-alpha" in docs.read_text(encoding="utf-8")
    assert "1.5.0-alpha" in (Path(__file__).resolve().parents[1] / "docs" / "AUTO_MARKETPLACE.md").read_text(
        encoding="utf-8"
    )
