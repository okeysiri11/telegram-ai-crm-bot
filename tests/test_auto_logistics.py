...
def test_version_modules_docs_bridges():
    ...
    assert docs.exists() and "4.1.2-enterprise" in docs.read_text(encoding="utf-8")
    assert "4.1.2-enterprise" in (Path(__file__).resolve().parents[1] / "docs" / "AUTO_MARKETPLACE.md").read_text(
        encoding="utf-8"
    )
