"""API handlers — Finance Enterprise Certification (Sprint 18.8)."""
...
def _suite():
    return finance_enterprise.enterprise_certification


async def fec_health_handler(request: web.Request) -> web.Response:
    ...
    return json_response(
        {
            ...
            "financial_integrity_certified": health.get("financial_integrity_certified"),
            "documentation_certified": health.get("documentation_certified"),
            "finance_enterprise_ready": health.get("finance_enterprise_ready"),
            "production_ready": health.get("production_ready"),
            "enterprise_release_ready": health.get("enterprise_release_ready"),
            "bidex_finance_enterprise_suite_released": health.get(
...
async def fec_bootstrap_handler(request: web.Request) -> web.Response:
    try:
...
async def fec_architecture_handler(request: web.Request) -> web.Response:
    try:
...
async def fec_integration_handler(request: web.Request) -> web.Response:
    try:
...
async def fec_performance_handler(request: web.Request) -> web.Response:
    try:
...
async def fec_security_handler(request: web.Request) -> web.Response:
    try:
...
async def fec_integrity_handler(request: web.Request) -> web.Response:
    try:
        return json_response(_suite().financial_integrity.certify())
    except Exception as exc:
        return _handle_error(exc)


async def fec_documentation_handler(request: web.Request) -> web.Response:
    try:
...
async def fec_quality_handler(request: web.Request) -> web.Response:
    try:
...
async def fec_release_handler(request: web.Request) -> web.Response:
    try:
...
async def fec_executive_handler(request: web.Request) -> web.Response:
    try:
...
async def fec_dashboard_handler(request: web.Request) -> web.Response:
    try:
        dtype = request.rel_url.query.get("type", "enterprise_health")
