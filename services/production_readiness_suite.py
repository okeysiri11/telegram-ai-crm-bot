...
def _git_revision() -> str:
    """Build/revision identity. Prefers injected deploy SHA over local git."""
    import os
    import subprocess
    from pathlib import Path

    for key in ("GIT_SHA", "SOURCE_REVISION", "GIT_REVISION", "GIT_COMMIT", "RENDER_GIT_COMMIT"):
        injected = (os.getenv(key) or "").strip()
