...
class NotificationEventHandler:
    ...
    async def handle(event: BaseEvent) -> None:
        ...
        elif isinstance(event, ManagerEscalationEvent):
            await NotificationEventHandler._on_escalation(event)
    ...
    @staticmethod
    async def _on_escalation(event: ManagerEscalationEvent) -> None:
        from services.notification_service import notification_service

        if event.manager_telegram_id is None:
            logger.info(
                "escalation_notification_skipped_no_manager",
                extra={"request_number": event.request_number},
            )
            return
        await notification_service.notify_managers_new_request(
            ...
            request_number=event.request_number,
            client_name="",
            product=f"🚨 Escalation L{event.escalation_level} ({event.overdue_seconds}s): {event.reason}",
            manager_telegram_id=event.manager_telegram_id,
        )
